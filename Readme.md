...
## Overview
This project applies a Genetic Algorithm (GA) to solve a combinatorial optimization problem similar to the traveling salesman problem (TSP). The objective is to optimize the foraging path of bees, ensuring they collect nectar from all flowers in a field in the least time possible, starting and ending at their hive located at coordinates (500, 500).
...
- **Euclidean Distance**: Used to calculate distances between points on the path. The distance `d` between two points (x1, y1) and (x2, y2) is calculated using the formula: 
